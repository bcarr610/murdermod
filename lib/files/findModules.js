...
const findModulesInDirectory = async (pathLike, ignorePaths, showLogs) => {
      ...
      if (items.length > 0) {
        ...
        await forEach(items, async item => {
          ...
          if (!splitPath.find(f => ignorePaths.includes(f))) {
            const foundModules = await findModulesInDirectory(path.join(pathLike, item), ignorePaths, showLogs);
